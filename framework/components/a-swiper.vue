<template>
	<view class="content" v-if="propsData.swiperList">
		<view v-if="propsData.swiperMode == 'image'">
			<u-swiper-dot :info="propsData.swiperList" :current="current" :mode="propsData.mode" :dotsStyles="propsData.dotsStylesList">
				<block slot="swiper">
					<swiper
						class="swiper-box"
						:autoplay="propsData.autoplay"
						:circular="propsData.circular"
						:duration="propsData.duration"
						:interval="propsData.interval"
						@change="change"
					>
		...
		<view v-if="propsData.swiperMode == 'text'" class="pl15 pr15 x-c">
			...
			<view class="pl5" @click="rightTextClickFun">
				<u-icons v-if="propsData.rightIcon" class="icon-box" :type="propsData.rightIcon"></u-icons>
	...
	methods: {
		...
		rightTextClickFun(){
			...
			if(this.propsData.swiperMode == "text" && this.propsData.type ){
				...
				if(!this.operateData.list.linkType){
					this.$R.push(operateDataList.link + coverParam);
					// console.log('url:', this.operateDataList.link + coverParam);
				}else {
					this.$R.jumpLink(this.operateDataList.link + coverParam);
