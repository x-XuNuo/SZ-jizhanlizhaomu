<template>
	<view ref="QS-SwiperGrid-items" class="x-ac QS-SwiperGrid-items" :style="{ padding: padding + 'px' }">
		<!-- #ifndef APP-NVUE -->
		<view 
...
<script>
	...
	export default {
		...
		computed: {
			getImageField() {
				return this.props.imageField || 'image'
			},
			getImageWidthField() {
		...
		mounted() {
			console.log("grids:",JSON.stringify(this.grids));
		},
		methods: {
			_click(index) {
...
<style scoped>
	/* #ifndef APP-NVUE */
	@import url("@/framework/components/ux-qs-grid/css/ux-qs-grid.css");
	/* #endif */
	.QS-SwiperGrid-items{
		/* #ifdef APP-NVUE */
		/* flex: 1; */
