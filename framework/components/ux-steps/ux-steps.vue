		...
		>
			<view class="u-steps__item" :class="['u-steps__item--' + direction]" v-for="(item, index) in list" :key="index">
				...
				>
					...
					<view v-else>
						<ux-icons v-if="iconMode == '0'" size="22" color="#ffffff" :type="icon" />
						<u-icon v-if="iconMode == '1'" size="22" color="#ffffff" :name="icon"></u-icon>
					</view>
				</view>
				<view
				...
				<block>
					<view v-if="stepMode == 1">
						<view class="u-steps__item__line u-stepsMode__item__line" :class="['u-steps__item__line--' + mode]" v-if="index < list.length - 1">
							<u-line v-if="index < current" :direction="direction" length="100%" :hair-line="false" :color="activeColor"></u-line>
						</view>
					</view>
...
@import '@/framework/libs/uView/libs/css/style.components.scss';
...
.u-steps {
	...
	.u-steps__item {
		...
		&__num {
			...
			line-height: $u-steps-item-number-width;
